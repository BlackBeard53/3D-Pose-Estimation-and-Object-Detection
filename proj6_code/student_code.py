...
def check_hand_inside_bounding_box(hand, pts):
    ...
    maximum_point = pts[7,:]
    #print(maximum_point)
    if hand[0]>=minimum_point[0] and hand[0]<=maximum_point[0] and hand[1]>=minimum_point[1] and hand[1]<=maximum_point[1] and hand[2]>=minimum_point[2] and hand[2]<=maximum_point[2]:
        inside = 1
    else:
        inside = 0
...
def process_video(path):
    ...
    def processFrame(sec):
        ...
        if hasFrames:
            ...
            projection_matrix = perspective_n_points(initial_box_points_3d,bounding_boxes, K)
            
            
            # Step-2: Call hand_pose_img to get landmarks from img_path
            landmarks = hand_pose_img(img_path)
            
            # Step-3: Project 2d landmakrs to 3d using given projection_depth and project matrix from step-3
            threeD_landmarks = projection_2d_to_3d(projection_matrix, depth, landmarks)
            
            # Step-4: Get hand coordinate from the index 22 (0-indexed) of 3d landmarks
            hand = threeD_landmarks[21]
           
            # Step-5 Call draw_box_intersection and get annotated_img
            annotated_img = draw_box_intersection(img,hand,threeD_landmarks,landmarks)
            ############################################################################
            #                             END OF YOUR CODE
            ############################################################################
